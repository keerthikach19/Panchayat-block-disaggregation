...
export default function ExplainabilityPanel({ explainData: p, loading }) {
  ...
  return <aside className="detail-panel"><span className="eyebrow">VILLAGE SOURCE DETAILS</span><h2>{p.panchayat_name}</h2><p>{p.block_name} block · {p.panchayat_id}</p><p>Issue: {p.issue_date || 'Unknown'}<br/>Valid: {p.valid_date}</p>
    ...
    {p.adjustment_details && <div className="terrain-details"><p>Copernicus elevation: {p.adjustment_details.village_elevation_m.toFixed(1)} m<br/>Rainfall factor: {p.adjustment_details.rainfall_factor.toFixed(3)}{p.temp_max_c != null && <><br/>Assumed parent elevation: {(p.adjustment_details.parent_reference_elevation_m ?? p.adjustment_details.block_reference_elevation_m).toFixed(1)} m<br/>Temperature adjustment: {p.adjustment_details.temperature_adjustment_c.toFixed(2)} °C</>}</p></div>}
    ...
    <table><caption>Source weather · no local adjustment</caption><thead><tr><th>Variable</th><th>Source value</th></tr></thead><tbody>{sourceFields.map(([k,n,u])=><tr key={k}><th>{n}</th><td>{p.source[k] == null ? '—' : p.source[k]+' '+u}</td></tr>)}</tbody></table>
