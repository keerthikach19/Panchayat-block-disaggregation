...
from src.ingestion.forecast_schema import ROOT, read_json, file_hash, digest, immutable_json, atomic_json
...
def context():
    ...
    out = ROOT/'data/research/benchmarks'/identity
...
def evaluate(fresh_dataset=None):
    config, meta, rain, out, _, _ = context()
    ...
    for label, dataset, values, years in datasets:
        ...
        for record in locked['by_lead']:
            ...
            old = lgb.Booster(model_file=str(old_dir/f'lead-{lead}.txt')).predict(X[:, :12], num_threads=4)
            ...
            np.savez_compressed(out/f'{label}-day-{lead}.npz', observed=y, original=old, amount=prediction,
                                event_probability=probability, event_alert=alert,
                                date=np.repeat(dates[times].strftime('%Y-%m-%d'), n)[test],
    ...
    atomic_json(ROOT/'data/research/benchmarks/event_evidence.json', dict(experiment_id=out.name))
...
if __name__ == '__main__':
    ...
    train() if args.stage == 'train' else evaluate(args.fresh_dataset)
